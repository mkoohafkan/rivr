...
$$
is zero. Both $y_n$ and $y_c$ are non-linear functions of $y$. `rivr` provides 
...
The output is formatted for easy visualization with `ggplot2`.
...
wave = floodwave = ifelse(times >= 9000, baseflow,
...
Accessing the unsteady flow outputs is greatly simplified by using `dplyr`.
